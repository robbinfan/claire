#ifndef _EXAMPLES_STDSORT_CLIENT_H_
#define _EXAMPLES_STDSORT_CLIENT_H_
...
class StdSortClient : boost::noncopyable
{
public:
    typedef boost::function<void(const std::vector<int32_t>&)> ResultCallback;
    StdSortClient(EventLoop* loop, const InetAddress& server_address)
      : channel_(loop),
        stub_(&channel_)
    {
    ...
    void SendRequest(protorpc::RpcControllerPtr& controller, const std::vector<int32_t>& v, const ResultCallback& done)
    {
        ...
        stub_.StdSort(controller, request, boost::bind(&StdSortClient::replied, this, _1, _2, done));
    }
...
private:
    ...
    sort::StdSortService::Stub stub_;
};

} // namespace claire

#endif // _EXAMPLES_STDSORT_CLIENT_H_
