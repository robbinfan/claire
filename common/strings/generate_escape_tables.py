...
OUTPUT_FILE = "EscapeTables.cc"

def generate(f):
    f.write("// Use of this source code is governed by a BSD-style license\n"
            "// that can be found in the License file.\n\n")

    f.write("namespace claire {\n"
            "\n")
    ...
    f.write("extern const unsigned char kHexTable[] = {")
    for i in range(0, 256):
        if i % 16 == 0:
            f.write("\n  ")
    ...
    f.write("extern const unsigned char kUriEscapeTable[] = {")
    passthrough = (
        range(ord('0'), ord('9')) +
        range(ord('A'), ord('Z')) +
        range(ord('a'), ord('z')) +
